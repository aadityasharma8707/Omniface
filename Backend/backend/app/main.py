...
async def _self_ping_loop() -> None:
    """
    Background keep-alive task for free cloud hosts (like Render).
    Every 10 minutes, performs an HTTP GET request to its public health endpoint.
    This routes traffic through Render's public edge router, preventing sleep/spin-down.
    """
    import httpx

    # Render automatically sets RENDER_EXTERNAL_URL to the public https:// url
    ping_url = os.getenv("RENDER_EXTERNAL_URL", "https://omniface-x6te.onrender.com")
    health_url = f"{ping_url.rstrip('/')}/health"

    # Wait 60 seconds after startup before initiating ping loop
    ...
    while True:
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                logger.info("Keep-alive self-ping: %s -> HTTP %s", health_url, res.status_code)
        except asyncio.CancelledError:
            break
        except Exception as exc:
            logger.debug("Keep-alive self-ping (non-fatal): %s", exc)

        try:
            # Ping every 10 minutes (600s), well before Render's 15-minute idle limit
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    Spawns background keep-alive self-ping loop.
    """
    ...
    # Start self-ping keep-alive loop to prevent free-tier host (Render) from sleeping
...
def create_app() -> FastAPI:
    ...
    # ── CORS ──────────────────────────────────────────────────────────────────
    # Allow localhost on any port, plus any vercel.app preview/production deployment
    _cors_origin_regex = (
