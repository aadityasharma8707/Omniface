...
class FusionEngine:
    ...
    def _passthrough(self, result: AnalyzerResult) -> FusionResult:
        ...
        if result.label == "uncertain":
            ...
            verdict = "real"
        elif result.label == "fake" and score > 0.50:
            # Respect definitive analyzer result above midpoint probability
            verdict = "fake"
        elif result.label == "real" and score < 0.50:
            # Respect definitive analyzer result below midpoint probability
            verdict = "real"
        else:
            verdict = "uncertain"

        if verdict == result.label and verdict in ("real", "fake") and not (score >= FAKE_THRESHOLD or score <= REAL_THRESHOLD):
            rationale = (
                f"Single-modality {result.modality} analysis yielded confidence={score:.3f}. "
                f"Definitive analyzer label '{result.label}' passed through without contradiction."
            )
        else:
            rationale = (
                f"Single-modality {result.modality} analysis yielded confidence={score:.3f}. "
