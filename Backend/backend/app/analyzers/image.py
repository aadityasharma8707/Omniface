...
class ImageAnalyzer(BaseAnalyzer):
    ...
    def _sensor_noise_score(self, img_np: np.ndarray, gray: np.ndarray, h: int, w: int) -> float:
        ...
        p35 = float(np.percentile(grad_mag, 35))
        threshold = max(p35, 4.0)
        ...
        if flat_noise.size < 64:
            return 0.20

        # Exclude extreme boundary jump pixels before computing moments
        if flat_noise.size > 256:
            p_lo, p_hi = np.percentile(flat_noise, [0.5, 99.5])
            eval_noise = flat_noise[(flat_noise >= p_lo) & (flat_noise <= p_hi)]
            if eval_noise.size < 64:
                eval_noise = flat_noise
        else:
            eval_noise = flat_noise

        noise_std = float(eval_noise.std())
        flat_var = float(eval_noise.var())
        
        if flat_var < 0.60:
            noise_kurtosis = 3.0
        else:
            noise_kurtosis = float(np.mean((eval_noise - eval_noise.mean())**4) / (flat_var**2 + 1e-6))
    ...
    def _compression_artifact_score(self, pil_img: Image.Image, gray: np.ndarray, h: int, w: int) -> float:
        ...
        if ela_gray.size == 0 or ela_gray.mean() < 1e-4:
            return 0.15
    ...
    def _facial_symmetry_score(self, gray: np.ndarray, h: int, w: int) -> float:
        """
        Facial boundary gradient coherence and bilateral structural consistency.
        """
        cy, cx = h // 2, w // 2
        bh, bw = max(h // 4, 16), max(w // 4, 16)
        roi_l = gray[max(cy - bh, 0) : min(cy + bh, h), max(cx - bw, 0) : cx]
        roi_r = gray[max(cy - bh, 0) : min(cy + bh, h), cx : min(cx + bw, w)]
        if roi_l.size > 0 and roi_r.size > 0 and roi_l.shape == roi_r.shape:
            roi_r_flip = np.fliplr(roi_r)
            diff = np.abs(roi_l - roi_r_flip)
            norm_factor = float(gray.std()) + 1e-4
            asym_metric = float(diff.mean()) / norm_factor
            return _logistic_score(asym_metric, x0=0.55, k=8.0, min_val=0.15, max_val=0.85)
        return 0.30
    ...
    def _corneal_reflection_score(self, img_np: np.ndarray, h: int, w: int) -> float:
        ...
        def find_highlights(roi: np.ndarray):
            ...
            if len(y_pts) == 0:
                return []
            # Corneal reflections are compact specular glints (< 4% of ROI or < 300 px)
            max_highlight_px = max(int(0.04 * roi.shape[0] * roi.shape[1]), 300)
            if len(y_pts) > max_highlight_px:
                return []
        ...
        if len(left_pts) >= 2 and len(right_pts) >= 2:
            ...
            return _logistic_score(y_diff, x0=0.12, k=20.0, min_val=0.15, max_val=0.88)
        else:
            r = img_f[:, :, 0].flatten()
            g = img_f[:, :, 1].flatten()
            b = img_f[:, :, 2].flatten()
