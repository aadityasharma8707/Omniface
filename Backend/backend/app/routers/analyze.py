...
def _detect_magic_bytes(b: bytes) -> Optional[str]:
    ...
    if b[:4] == b"OggS":
        return "audio/ogg"
    # MP4 / MOV / M4A (ISO Base Media File)
    if len(b) >= 8 and (b[4:8] == b"ftyp" or b[4:8] == b"moov"):
        if len(b) >= 12 and b[8:12] in (b"M4A ", b"M4B ", b"M4P "):
            return "audio/mp4"
        return "video/mp4"
    # WebM / MKV
...
def _detect_modality(mime_type: str, filename: str) -> str:
    """
    Detect modality from filename extension and MIME type.
    Filename extensions take priority for container formats like MP4/M4A.
    ...
    if ext in ("m4a", "aac", "mp3", "wav", "flac", "ogg", "opus", "weba", "wma", "wave", "aiff", "caf", "amr") or \
       any(tag in lower_name for tag in (".m4a.", ".mp3.", ".wav.", ".aac.", ".flac.", ".ogg.")):
        return "audio"
    if ext in ("jpg", "jpeg", "png", "webp", "gif", "bmp", "tiff", "svg", "heic"):
        return "image"
    if ext in ("mp4", "mov", "webm", "mkv", "avi", "m4v", "3gp", "flv"):
        return "video"
    ...
    raise HTTPException(
        ...
        detail=f"Unsupported MIME type: '{mime_type}'. "
               f"Accepted types: image (jpeg/png/webp), audio (mp3/wav/m4a/flac), video (mp4/webm/mov).",
    )
...
async def analyze_media(
    ...
    modality: Optional[str] = Form(None, description="Explicit modality ('image' | 'audio' | 'video')"),
    ...
    resolved_modality = (
        modality.lower()
        if modality and modality.lower() in ("image", "audio", "video")
        else _detect_modality(mime_type, file_name)
    )
