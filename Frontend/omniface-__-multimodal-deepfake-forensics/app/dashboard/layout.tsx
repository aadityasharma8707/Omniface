...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const [isLoadingAuth, setIsLoadingAuth] = useState(() => !authService.getCurrentUser());
  const [isCollapsed, setIsCollapsed] = useState(false);

  // Initialize sidebar collapse state based on viewport or stored preference
  useEffect(() => {
    try {
      const saved = localStorage.getItem('omniface_sidebar_collapsed');
      if (saved !== null) {
        setIsCollapsed(saved === 'true');
      } else if (window.innerWidth >= 768 && window.innerWidth < 1024) {
        // Auto-collapse on tablet to give more canvas space
        setIsCollapsed(true);
      }
    } catch {
      // ignore
  ...
  const toggleSidebar = () => {
    setIsCollapsed((prev) => {
      const next = !prev;
      try {
        localStorage.setItem('omniface_sidebar_collapsed', String(next));
      } catch {
        // ignore
      }
      return next;
  ...
  useEffect(() => {
    const apiUrl = (process.env.NEXT_PUBLIC_API_URL || 'https://omniface-x6te.onrender.com').replace(/\/+$/, '');
    const ping = () => {
  ...
  return (
    <div style={{
      ...
      */}
      ...
      {/* 
        =============================================================================
        SIDEBAR NAVIGATION (DESKTOP & TABLET: EXPANDABLE / COLLAPSIBLE)
        =============================================================================
      */}
      <aside
        ...
        className="omniface-desktop-sidebar"
      >
        ...
        <div>
          <div style={{
            ...
            <Link
              href="/"
              title="OmniFace Forensic Laboratory"
              style={{
                display: 'flex',
                alignItems: 'center',
                gap: '10px',
            ...
            <button
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.backgroundColor = 'rgba(0, 229, 255, 0.18)';
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon size={18} color={isActive ? '#00E5FF' : 'currentColor'} style={{ flexShrink: 0 }} />
        ...
        <div style={{
          ...
          <div
            title="Neural Forensic Engine: ONLINE (60 FPS SYNC)"
            style={{
          ...
          <div style={{
            ...
            <div
              title={`${user?.name || 'Analyst'} (${user?.email || 'analyst@omniface.ai'})`}
      ...
      {/* 
        =============================================================================
        MOBILE NAVIGATION HEADER & OFF-CANVAS DRAWER WITH BACKDROP
        =============================================================================
      */}
      {/* Mobile Top App Bar */}
      <div
      ...
      >
        <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none' }}>
          ...
          <span style={{ fontFamily: 'var(--f-display)', fontSize: '14px', fontWeight: 800, letterSpacing: '0.06em', color: '#FFFFFF' }}>
            OMNIFACE
          </span>
        </Link>
      ...
      {mobileMenuOpen && (
        <div
          ...
          className="omniface-mobile-drawer"
        />
      )}
        ...
        <div>
          ...
          <div style={{
            ...
            >
              ...
              <div>
                <div style={{ fontFamily: 'var(--f-display)', fontSize: '13px', fontWeight: 700, color: '#FFFFFF' }}>
                  OMNIFACE
                </div>
                <div style={{ fontFamily: 'var(--f-mono)', fontSize: '9px', color: '#00E5FF' }}>
                  DEEPFAKE FORENSICS
                </div>
              </div>
            </Link>
              ...
              return (
                <Link
                  ...
                  style={{
                    ...
                    border: isActive ? '1px solid rgba(0, 229, 255, 0.35)' : '1px solid rgba(255, 255, 255, 0.05)',
                    minHeight: '44px',
                  }}
                >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', borderTop: '1px solid rgba(255, 255, 255, 0.08)', paddingTop: '16px' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontFamily: 'var(--f-mono)', fontSize: '10px', color: '#A0AEC0' }}>Neural Engine</span>
            </div>
            <span style={{ fontFamily: 'var(--f-mono)', fontSize: '10px', color: '#10B981', fontWeight: 700 }}>ONLINE</span>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div style={{ minWidth: 0, overflow: 'hidden' }}>
              <div style={{ fontWeight: 600, color: '#FFFFFF', fontSize: '13px', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                {user?.name || 'Analyst'}
              </div>
              <div style={{ fontSize: '11px', color: 'rgba(255, 255, 255, 0.5)', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                {user?.email || 'analyst@omniface.ai'}
              </div>
            </div>
          </div>
      ...
      {/* 
        =============================================================================
        MAIN CONTENT AREA (ADAPTIVE MARGIN-LEFT)
        =============================================================================
      */}
      <div
        ...
        className={`omniface-main-area ${isCollapsed ? 'sidebar-collapsed' : 'sidebar-expanded'}`}
      >
        {/* Top bar (Desktop) */}
        <header
          style={{
          ...
          className="omniface-desktop-header"
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <main
          ...
          className="omniface-main-content"
        >
          {children}
        </main>
      </div>
