...
function _setSessionCookie(): void {
  ...
  document.cookie = `omniface_session=1; path=/; max-age=2592000; SameSite=Lax${isSecure ? '; Secure' : ''}`;
}
  ...
  if (typeof error === 'object' && error !== null && 'code' in error) {
    ...
    switch (authErr.code) {
      ...
      case 'auth/api-key-not-valid':
        return 'Firebase API key is invalid or Authentication is not enabled in Firebase Console. Please click "Get Started" in Firebase Console > Build > Authentication and restart your dev server.';
      default:
