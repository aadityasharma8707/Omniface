...
export const analysisService = {
  ...
  analyzeMedia: async (
    ...
    formData.append('modality', mediaType);
    ...
    let res: Response;
    try {
      res = await fetch(`${BACKEND_API_URL}/api/v1/analyze`, {
        method: 'POST',
        headers: {
          ...authHeaders,
        },
      ...
      if (fetchErr?.message === 'Failed to fetch' || fetchErr?.name === 'TypeError') {
        throw new Error(
          `Unable to connect to backend (${BACKEND_API_URL}). The Render server may be waking up from sleep mode (takes ~45-60s on free tier) or CORS is blocking the request. Please test https://omniface-x6te.onrender.com/health directly in your browser.`
        );
      }
      throw fetchErr;
    }
